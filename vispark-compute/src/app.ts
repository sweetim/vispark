...
; (async () => {
    ...
    const wallet = new ethers.Wallet(
        process.env.PRIVATE_WALLET_KEY, provider);
    ...
    const { endpoint, model } = await broker.inference.getServiceMetadata(providerAddress_llama);
    console.log({ endpoint, model })

    const questions = "you are an expert of summarize, please summarize the content below"
    ...
    const response = await fetch(`${endpoint}/chat/completions`, {
        ...
        body: JSON.stringify({
            messages: [
                ...
                { role: "user", content: transcipts.map(item => item.text).join(" ") }
            ],
            model: model,
        }),
    });
